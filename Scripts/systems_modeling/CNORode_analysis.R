...
cnolist = CNOlist("99quant_per_Pep_cleaned.csv")

pknmodel = readSIF("sigUpdate.SIF") #THIS IS THE ONE USED IN THE PAPER
...
#FIND OPTIMIZATIONS AND PLOT THE PLOTFITS
optT1 = gaBinaryT1(CNOlistN, model, verbose=FALSE)
optT2 = gaBinaryTN(CNOlistN, model, verbose=FALSE, bStrings=list(optT1$bString))
optT3 = gaBinaryTN(CNOlistN, model, verbose=FALSE, bStrings=list(optT1$bString, optT2$bString))
optT4 = gaBinaryTN(CNOlistN, model, verbose=FALSE, bStrings=list(optT1$bString, optT2$bString, optT3$bString))
...
simulatedData=plotLBodeFitness(CNOlistN, pknmodel,initial_pars)
...
dir.create("V2")
pdf("V2/Optimization_results_final.pdf", width = 40, height = 20)
...
ggsave("V2/k_and_tau_transtest_final.pdf", width = 11, height = 6)
...
pdf("V2/RMSE_plots_CNORode_final.pdf", width = 15, height = 9)
map
dev.off()
...
ggsave("Scatterplot_measured_predicted.pdf", width = 14, height = 9)
...
#The formula is
#measuredValues = simulatedValues * 0.5951 + 0.1786 (0.095)
0.5951 * 0 + 0.1786 +0.095
#x = c(0, 0, 1, 1)
#y = c(0.057, 0.257, 0.7365, 0.9365)
shapeDF <- data.frame(x  = c(0, 1, 1, 0), y =c(0.0836, 0.6787, 0.8687, 0.2736))
